...
class Pengesahan extends CI_Controller
{
    ...
    public function agenda_number($surat, $pengesahan)
    {
        if ($this->session->userdata('role') != 'Pembuat') redirect('welcome');
        if ($this->input->post('submit')) {
            $this->Pengesahan_model->agenda_number($surat, $pengesahan);
            $this->session->set_flashdata('msg', '
            <div id="alert" class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
                Anda sudah mengisi nomor agenda, QRcode sudah ada!
            </div>
            ');
            redirect('surat');
        }
        $data['title'] = 'Nomor Agenda';
        $data['validation'] = $this->Pengesahan_model->read_by_id($pengesahan);
